...
const availableVehicles = ['teslax', 'MODELS'];
...
class Autopilot {
	...
	toggle() {
		...
		if (!vehicle) {
			return mp.game.ui.notifications.error('Сядьте в ТС');
		}
	...
	private start(vehicle: VehicleMp) {
		if (!waypoint.isActive()) {
			return mp.game.ui.notifications.error('Нужно указать точку на карте');
		}

		if (!this.isAvailableVehicle(vehicle)) {
			return mp.game.ui.notifications.error('Данное ТС, не имеет поддержки автопилота');
		}
		...
		mp.game.ui.notifications.info('Автопилот активирован');
	}
	...
	private stop(vehicle: VehicleMp) {
		...
		mp.game.ui.notifications.info('Автопилот отключён');
	}
