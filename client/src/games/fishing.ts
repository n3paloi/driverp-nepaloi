...
class Fishing {
	...
	start() {
		...
		mp.game.ui.notifications.info('Кажется, что-то клюнуло!');
	}
	...
	private async getFish() {
		this.stop();
		await rpc.callServer('Fishing-Success');
	}

	private async dropFish() {
		this.stop();
		await rpc.callServer('Fishing-Drop');

		mp.game.ui.notifications.info('Черт, рыба сорвалась...');
	}
	...
	private async checkBarPosition() {
